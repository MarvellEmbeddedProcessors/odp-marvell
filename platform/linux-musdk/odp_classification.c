			     ...
			     odp_cos_t cos_table[])
{
	...
	if (num_qos > ODP_COS_MAX_L2_QOS) {
		ODP_ERR("Invalid num_qos. valid range 0 - %d\n",
			ODP_COS_MAX_L2_QOS);
		return -1;
	}
	...
	for (i = 0; i < num_qos; i++) {
		...
		if ((cos != NULL) && (ODP_COS_MAX_L2_QOS > qos_table[i])) {
			l2_cos->cos[qos_table[i]] = cos;
			l2_cos->cos_by_idx[i] = cos;
		}
	}

	if (entry->s.ops->cos_with_l2_priority)
		ret = entry->s.ops->cos_with_l2_priority(entry,
							 num_qos,
							 qos_table);
	pktio_cls_enabled_set(entry, 1);
	UNLOCK(&l2_cos->lock);
			...
			odp_bool_t l3_preference)
{
	...
	for (i = 0; i < num_qos; i++) {
		cos = get_cos_entry(cos_table[i]);
		if ((cos != NULL) && (ODP_COS_MAX_L3_QOS > qos_table[i])) {
			l3_cos->cos[qos_table[i]] = cos;
			l3_cos->cos_by_idx[i] = cos;
		}
	}

	if (entry->s.ops->cos_with_l3_priority)
		ret = entry->s.ops->cos_with_l3_priority(entry,
							 num_qos,
							 qos_table);
	pktio_cls_enabled_set(entry, 1);
	UNLOCK(&l3_cos->lock);
